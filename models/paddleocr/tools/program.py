...
def train(
    ...
    model_average = False
    # print("Starting main train")
    model.train()
    # print("putting model to train")
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader):
            # print("getting data idx", idx, len(batch))
            ...
            if scaler:
                with paddle.amp.auto_cast(
                    ...
                    # print("Passing image to the model", model_type, algorithm)
                    if model_type == "table" or extra_input:
                        # print("Starting forward pass1")
                        # print(batch[1:])
                        preds = model(images, data=batch[1:])
                    elif model_type in ["kie"]:
                        # print("Starting forward pass2")
                        preds = model(batch)
                    elif algorithm in ["CAN"]:
                        # print("Starting forward pass3")
                        preds = model(batch[:3])
                    else:
                        # print("Starting forward pass4")
                        preds = model(images)
